...
train_root = _read_yaml_scalar(CONFIG_YAML, 'father_train_path_npz')
if train_root:
    opt.father_train_path_npz = train_root

val_root = _read_yaml_scalar(CONFIG_YAML, 'father_val_path_npz')
if val_root:
    opt.father_val_path_npz = val_root

PRETRAINED_PATH = _read_yaml_scalar(CONFIG_YAML, 'pretrained_path')
...
def main():
    ...
    if PRETRAINED_PATH and (not opt.TRAINING.RESUME):
